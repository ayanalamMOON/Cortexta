...
async function runContract(name: string, factory: DaemonFactory): Promise<void> {
    await withServer(name, factory, async (baseUrl) => {
        const sessionProjectPath = fs.mkdtempSync(path.join(os.tmpdir(), "cortexa-daemon-session-resurrection-"));
        fs.writeFileSync(
            path.join(sessionProjectPath, "session-resurrection-sample.ts"),
            [
                "export function sessionResurrectionProbe(value: number): number {",
                "  return value * 2;",
                "}",
                "",
                "export const sessionResurrectionLabel = 'daemon-integration';"
            ].join("\n"),
            "utf8"
        );
        ...
        try {
            ...
            assert.equal(
                typeof healthBody.sessionResurrection?.enabled,
                "boolean",
                `[${name}] /health sessionResurrection.enabled`
            );
            assert.equal(
                typeof healthBody.sessionResurrection?.started,
                "boolean",
                `[${name}] /health sessionResurrection.started`
            );
            assert.equal(
                typeof healthBody.sessionResurrection?.running,
                "boolean",
                `[${name}] /health sessionResurrection.running`
            );
            assert.equal(
                typeof healthBody.sessionResurrection?.runCount,
                "number",
                `[${name}] /health sessionResurrection.runCount`
            );
            assert.equal(
                typeof healthBody.sessionResurrection?.consecutiveFailures,
                "number",
                `[${name}] /health sessionResurrection.consecutiveFailures`
            );
            assert.equal(
                typeof healthBody.sessionResurrection?.slo?.generatedAt,
                "number",
                `[${name}] /health sessionResurrection.slo.generatedAt`
            );
            assert.ok(
                Array.isArray(healthBody.sessionResurrection?.slo?.windows),
            ...
            const agentRunMissingInput = await postJson(baseUrl, "/cxlink/agent/run", {}, TOKEN);
            await expectStatus(agentRunMissingInput, 400, `[${name}] /cxlink/agent/run missing fields`);

            const agentRunInvalidAgent = await postJson(
                baseUrl,
                "/cxlink/agent/run",
                { agent: "not_real", text: "test" },
                TOKEN
            );
            await expectStatus(agentRunInvalidAgent, 400, `[${name}] /cxlink/agent/run invalid agent`);
            ...
            const agentListBody = (await agentListResponse.json()) as {
                ...
                route?: string;
                count?: number;
                agents?: Array<{ id?: string; family?: string; mutation?: boolean; description?: string }>;
            };
            assert.equal(agentListBody.ok, true, `[${name}] /cxlink/agent/list ok`);
            assert.equal(agentListBody.route, "cxlink/agent/list", `[${name}] /cxlink/agent/list route`);
            assert.equal(typeof agentListBody.count, "number", `[${name}] /cxlink/agent/list count`);
            assert.ok(Array.isArray(agentListBody.agents), `[${name}] /cxlink/agent/list agents array`);
            assert.ok(
                (agentListBody.agents ?? []).some((agent) => agent.id === "planner"),
                `[${name}] /cxlink/agent/list includes planner`
            );

            const agentRunResponse = await postJson(
                baseUrl,
                "/cxlink/agent/run",
                {
                    projectId: "daemon-agent-integration",
                    branch: "main",
                    agent: "multi_agent_loop",
                    text: "design a safe rollout plan for agent orchestration",
                    dryRun: true,
                    topK: 8,
                    maxChars: 512
                },
            ...
            await expectStatus(agentRunResponse, 200, `[${name}] /cxlink/agent/run success`);
            const agentRunBody = (await agentRunResponse.json()) as {
                ok?: boolean;
                route?: string;
                agent?: string;
                projectId?: string;
            ...
            assert.equal(agentRunBody.ok, true, `[${name}] /cxlink/agent/run ok`);
            assert.equal(agentRunBody.route, "cxlink/agent/run", `[${name}] /cxlink/agent/run route`);
            assert.equal(agentRunBody.agent, "multi_agent_loop", `[${name}] /cxlink/agent/run agent`);
            assert.equal(agentRunBody.projectId, "daemon-agent-integration", `[${name}] /cxlink/agent/run projectId`);
            assert.equal(agentRunBody.branch, "main", `[${name}] /cxlink/agent/run branch`);
            assert.equal(agentRunBody.dryRun, true, `[${name}] /cxlink/agent/run dryRun`);
            assert.equal(typeof agentRunBody.result, "object", `[${name}] /cxlink/agent/run result object`);
            assert.equal(
                agentRunBody.streamEvent?.payload?.eventType,
                "agentStatus",
                `[${name}] /cxlink/agent/run stream event type`
            );
            ...
            const sessionResStatusResponse = await postJson(baseUrl, "/cxlink/session-resurrection/status", {}, TOKEN);
            await expectStatus(sessionResStatusResponse, 200, `[${name}] /cxlink/session-resurrection/status success`);
            const sessionResStatusBody = (await sessionResStatusResponse.json()) as {
                ok?: boolean;
                route?: string;
                status?: {
                    enabled?: boolean;
            ...
            assert.equal(sessionResStatusBody.ok, true, `[${name}] /cxlink/session-resurrection/status ok`);
            assert.equal(
                sessionResStatusBody.route,
                "cxlink/session-resurrection/status",
                `[${name}] /cxlink/session-resurrection/status route`
            );
            assert.equal(
                typeof sessionResStatusBody.status?.enabled,
                "boolean",
                `[${name}] /cxlink/session-resurrection/status enabled`
            );
            assert.equal(
                typeof sessionResStatusBody.status?.runCount,
                "number",
                `[${name}] /cxlink/session-resurrection/status runCount`
            );
            assert.equal(
                typeof sessionResStatusBody.status?.consecutiveFailures,
                "number",
                `[${name}] /cxlink/session-resurrection/status consecutiveFailures`
            );
            assert.ok(
                Array.isArray(sessionResStatusBody.status?.recentRuns),
                `[${name}] /cxlink/session-resurrection/status recentRuns`
            );
            assert.equal(
                typeof sessionResStatusBody.status?.slo?.generatedAt,
                "number",
                `[${name}] /cxlink/session-resurrection/status slo.generatedAt`
            );
            assert.ok(
                Array.isArray(sessionResStatusBody.status?.slo?.windows),
                `[${name}] /cxlink/session-resurrection/status slo.windows`
            );

            const sessionResTriggerResponse = await postJson(
                baseUrl,
                "/cxlink/session-resurrection/trigger",
                {
                    reason: "integration-test",
                    dryRunOnly: true,
                    includeChats: false,
            ...
            await expectStatus(sessionResTriggerResponse, 200, `[${name}] /cxlink/session-resurrection/trigger success`);
            const sessionResTriggerBody = (await sessionResTriggerResponse.json()) as {
                ok?: boolean;
                route?: string;
                report?: {
                    trigger?: string;
            ...
            assert.equal(sessionResTriggerBody.ok, true, `[${name}] /cxlink/session-resurrection/trigger ok`);
            assert.equal(
                sessionResTriggerBody.route,
                "cxlink/session-resurrection/trigger",
                `[${name}] /cxlink/session-resurrection/trigger route`
            );
            assert.equal(
                sessionResTriggerBody.report?.trigger,
                "manual",
                `[${name}] /cxlink/session-resurrection/trigger report.trigger`
            );
            assert.equal(
                sessionResTriggerBody.report?.dryRunOnly,
                true,
                `[${name}] /cxlink/session-resurrection/trigger report.dryRunOnly`
            );
            assert.equal(
                typeof sessionResTriggerBody.report?.outcome,
                "string",
                `[${name}] /cxlink/session-resurrection/trigger report.outcome`
            );
            assert.equal(
                typeof sessionResTriggerBody.report?.ingestion?.filesScanned,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger report.ingestion.filesScanned`
            );
            assert.equal(
                typeof sessionResTriggerBody.report?.graphIndex?.nodesUpserted,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger report.graphIndex.nodesUpserted`
            );
            assert.equal(
                typeof sessionResTriggerBody.report?.graphIndex?.edgesUpserted,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger report.graphIndex.edgesUpserted`
            );
            assert.equal(
                typeof sessionResTriggerBody.report?.audit?.scannedRows,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger report.audit.scannedRows`
            );
            assert.ok(
                Array.isArray(sessionResTriggerBody.report?.decision?.reasons),
                `[${name}] /cxlink/session-resurrection/trigger report.decision.reasons`
            );
            assert.equal(
                typeof sessionResTriggerBody.status?.runCount,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger status.runCount`
            );
            assert.equal(
                typeof sessionResTriggerBody.status?.consecutiveFailures,
                "number",
                `[${name}] /cxlink/session-resurrection/trigger status.consecutiveFailures`
            );
            assert.equal(
                sessionResTriggerBody.streamEvent?.payload?.eventType,
                "sessionResurrectionStatus",
                `[${name}] /cxlink/session-resurrection/trigger stream event type`
            );
            ...
            fs.rmSync(sessionProjectPath, { recursive: true, force: true });
        }
    });
}
